# POSEIDEN web instance - multi-stage build for a slim final image.
#
# Stage 1 compiles the server binary (the whole workspace is available, but we
# only build `poseiden-server`). Stage 2 is a minimal Debian slim runtime with
...
# makes `poseiden.sh up`/`verify` and `skaffold dev` fast on every edit.
...
# change, so a source-only edit skips straight past it. Scoped to poseiden-server:
# the workspace also contains poseiden-app (Tauri), whose deps need GTK/WebKit
# system libraries this slim builder doesn't have.
COPY --from=planner /build/recipe.json recipe.json
RUN cargo chef cook --release -p poseiden-server --recipe-path recipe.json
...
RUN cargo build --release -p poseiden-server --bin poseiden-server
...
WORKDIR /build
ENV POSEIDEN_STATIC_DIR=/build/frontend/web \
    POSEIDEN_DATA_DIR=/data \
    POSEIDEN_BIND_ADDR=0.0.0.0 \
    RUST_LOG=info
EXPOSE 8737
...
CMD ["cargo", "watch", "--poll", "-w", "crates", "-x", "run -p poseiden-server --bin poseiden-server"]
...
RUN useradd --system --uid 10001 --home /nonexistent --shell /usr/sbin/nologin poseiden \
    && mkdir -p /data \
    && chown poseiden:poseiden /data

COPY --from=builder /build/target/release/poseiden-server /usr/local/bin/poseiden-server
...
ENV POSEIDEN_STATIC_DIR=/app/frontend/web \
    POSEIDEN_DATA_DIR=/data \
    POSEIDEN_BIND_ADDR=0.0.0.0 \
    RUST_LOG=info
...
# a space (poseiden.sh passes "YYYYMMDD HH:MM") survives ENV parsing intact.
# CI's docker.yml passes the git sha; local builds pass the timestamp.
ARG POSEIDEN_VERSION=latest-main
ARG POSEIDEN_COMMIT=unknown
ENV POSEIDEN_VERSION="$POSEIDEN_VERSION"
ENV POSEIDEN_COMMIT="$POSEIDEN_COMMIT"
...
ENTRYPOINT ["/usr/local/bin/poseiden-server"]
