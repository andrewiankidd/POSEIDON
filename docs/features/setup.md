...
Getting started: choose how to run POSEIDON on first launch, then - if you're
...
On first launch POSEIDON asks how you want to run it.
...
- **Connect to a shared instance** - point the client at a POSEIDON instance your
...
When POSEIDON is served from a static host (its GitHub Pages site), the landing
...
everyone else connects to). POSEIDON reads from three providers - **Azure DevOps,
...
- **GitHub / GitLab** read **public** repos with no sign-in at all - leave the
  Token env var blank and POSEIDON polls anonymously. For **private** repos (or to
...
Entirely **optional**. POSEIDON can suggest tags with an AI model, but the
...
`poseidon poll` runs one poll across every configured team and reports what it
fetched - handy for a first-run smoke test or a scheduled refresh. It uses the
same data directory as the app (`POSEIDON_DATA_DIR`, or the portable `.portable`
folder).
...
all. Then `poseidon poll`. Seed a data directory declaratively (CI, a fresh
instance, a new tenant) with `poseidon config import <bundle>.poseidon.import.yaml`
...
- **Config** - no config file. Teams, rules, tags, and saved reports live in the
  **database** (per owner); back them up, share them, or seed them with `poseidon
...
- **Secrets** - a provider token is read from the environment variable named by
  the team's **Token env var** (Azure DevOps defaults to `POSEIDON_AZURE_PAT`),
