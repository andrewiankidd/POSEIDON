#!/usr/bin/env bash
# POSEIDEN - the one dev CLI for the project: run the stack, and build artifacts.
#
#   run the minikube stack:
#     ./poseiden.sh up                # build -> deploy the full stack -> seed tenants/ -> open the app
#     ./poseiden.sh dev               # live-reload loop (skaffold: rebuild + redeploy on save)
#     ./poseiden.sh down [--clean]    # uninstall the release (--clean also stops minikube)
#     ./poseiden.sh verify            # deploy an isolated release, assert the stack, tear it down
...
#     ./poseiden.sh build all         # cli + desktop + image
#   (the stack verbs also work under 'run', e.g. './poseiden.sh run up')
#
...
# `owner:` field; owner-less falls back to POSEIDEN_OWNER) and opens the web UI.
...
# POSEIDEN_OWNER (fallback owner), POSEIDEN_IMAGE_TAG, POSEIDEN_RELEASE /
# POSEIDEN_HOST / POSEIDEN_VALUES.
set -euo pipefail

HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CHART="$HERE/deploy/helm/poseiden"
TENANTS="$HERE/tenants"
APP_TAURI_CONF="$HERE/crates/poseiden-app/tauri.conf.json"   # desktop/android builds
RELEASE="${POSEIDEN_RELEASE:-poseiden}"
HOST="${POSEIDEN_HOST:-poseiden.localhost}"
VALUES="${POSEIDEN_VALUES:-$CHART/ci/minikube-dex-values.yaml}"
FALLBACK_OWNER="${POSEIDEN_OWNER:-admin@example.com}"
IMAGE_REPO="poseiden"
IMAGE_TAG="${POSEIDEN_IMAGE_TAG:-dev}"
IMAGE="$IMAGE_REPO:$IMAGE_TAG"
INGRESS_PIDFILE="${TMPDIR:-/tmp}/poseiden-ingress-pf.pid"
...
# POSEIDEN_MINIKUBE_CPUS / _MEMORY.
MK_CPUS="${POSEIDEN_MINIKUBE_CPUS:-8}"
MK_MEMORY="${POSEIDEN_MINIKUBE_MEMORY:-8192}"
  ...
  if minikube status >/dev/null 2>&1; then
    ...
    if [ "${cur_cpus:-2}" -lt "$MK_CPUS" ] 2>/dev/null; then
      ...
      if [ -t 0 ]; then
        ...
        warn "Non-interactive - leaving it. Raise it with: $hint  (or set POSEIDEN_MINIKUBE_CPUS/_MEMORY)."
      fi
    fi
  else
...
deploy() {
  ...
  docker build --provenance=false --target runtime \
    --build-arg "POSEIDEN_COMMIT=$(date '+%Y%m%d %H:%M')" \
    -t "$IMAGE" "$HERE"
  ...
  # metachars in owners like poseiden+demo-data@…).
  ...
  shopt -s nullglob
  for f in "$TENANTS"/*.poseiden.import.yaml; do
  ...
  # (poseiden:dev), so a code-only change leaves the Deployment manifest identical
...
# Import every tenants/*.poseiden.import.yaml into its OWN tenant. The owner comes
...
import_tenants() {
  shopt -s nullglob
  local files=("$TENANTS"/*.poseiden.import.yaml)
  ...
  for f in "${files[@]}"; do
    ...
    curl -fsS -X POST "$BASE/api/settings/poll-all-teams" \
      -H "X-Auth-Request-Email: $owner" -H "Content-Type: application/json" \
      -d '{"all":true}' >/dev/null || true
    curl -fsS -X POST "$BASE/api/poll" -H "X-Auth-Request-Email: $owner" >/dev/null \
      || warn "  poll for $owner had errors (a real-provider team may need POSEIDEN_AZURE_PAT)"
  done
...
run_assertions() {
  local base="$1" bundle="$TENANTS/demo-data.poseiden.import.yaml"
  local alice="poseiden+e2e@example.com" bob="poseiden+e2e-other@example.com"
  info "health"
...
up() {
  ...
  if kubectl get "deploy/$RELEASE-hub" >/dev/null 2>&1; then
    ...
    target="http://$HOST"
    echo "  Log in (demo tenant):  poseiden+demo-data@example.com / password"
  ...
  if ingress_forward_start; then
    ...
    echo "  './poseiden.sh down' stops it. (*.localhost resolves to 127.0.0.1 in Chrome/Edge.)"
  else
...
verify() {
  RELEASE="poseiden-verify"
  HOST="poseiden-verify.localhost"       # unique host so it can't collide with a dev's release
  require docker minikube helm kubectl curl
...
build_cli() {
  ...
  echo "  + cargo build --release -p poseiden-cli"
  cargo build --release -p poseiden-cli
  info "CLI binary: target/release/poseiden"
}
...
build_apk() {
  require cargo cargo-tauri
  if [ ! -d "$HERE/crates/poseiden-app/gen/android" ]; then
    warn "Android project not initialized (crates/poseiden-app/gen/android missing)."
  ...
  info "APK under crates/poseiden-app/gen/android/app/build/outputs/."
}
...
build() {
  case "${1:-}" in
    ...
    ""|*)    die "usage: ./poseiden.sh build {image | desktop | apk | cli | web | all}" ;;
  esac
}

usage() {
  cat <<'EOF'
POSEIDEN dev CLI - usage: ./poseiden.sh <command>
  ...
  build artifacts (same commands CI runs - see docs/DISTRIBUTION.md):
    build cli        release CLI binary (target/release/poseiden)
