...
Deploys the POSEIDON web instance (single-replica, SQLite on a PV) with **optional
...
The examples below install from the source tree (`deploy/helm/poseidon`) for
...
# open http://poseidon.localhost  → login admin@example.com / password
```
...
meshConfig:
  extensionProviders:
    - name: poseidon-oauth2-proxy                 # matches AuthorizationPolicy provider.name
      envoyExtAuthzHttp:
        service: poseidon-oauth2-proxy.<ns>.svc.cluster.local
        port: 80
...
POSEIDON can suggest canonical tags for work items from each team's approved set.
...
**Injected env.** When `ai.enabled`, the deployment sets `POSEIDON_AI_ENDPOINT`
(the bundled Ollama service URL, or `ai.endpoint`), `POSEIDON_AI_MODEL`, and
`POSEIDON_AI_API_KEY` (hosted, from the Secret). It always sets `HF_HOME=/models`
...
| Key | Default | Description |
|-----|---------|-------------|
| `image.repository` / `image.tag` | `ghcr.io/andrewiankidd/poseidon` / chart appVersion | App image. |
...
| `pollInterval` | `15m` | Poll cadence (env `POSEIDON_POLL_INTERVAL`). |
| `telemetry.otlpEndpoint` | `""` | OTLP collector URL; set to enable export. |
| `ingress.type` | `nginx` | `nginx` \| `istio`. |
| `ingress.host` | `poseidon.example.com` | Ingress/VirtualService host. |
