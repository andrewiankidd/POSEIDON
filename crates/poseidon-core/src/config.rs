...
/// Instance-level POSEIDON configuration, sourced entirely from the environment
...
pub struct PoseidonConfig {
    ...
    /// host binary reads this and calls `poseidon_telemetry::init`.
    pub telemetry: poseidon_telemetry::TelemetrySettings,
}
...
impl UserConfig {
    ...
    /// default. Mirrors [`PoseidonConfig::rules_for`].
...
impl ServerConfig {
    ...
    /// `POSEIDON_BIND_ADDR`, `POSEIDON_PORT`, `POSEIDON_POLL_INTERVAL`.
    pub fn from_env() -> Self {
        let mut s = Self::default();
        if let Ok(v) = std::env::var("POSEIDON_BIND_ADDR") {
            if !v.trim().is_empty() {
                s.bind_addr = v;
            }
        }
        if let Ok(v) = std::env::var("POSEIDON_PORT") {
            if let Ok(p) = v.trim().parse() {
                s.port = p;
            }
        }
        if let Ok(v) = std::env::var("POSEIDON_POLL_INTERVAL") {
            if !v.trim().is_empty() {
                s.poll_interval = v;
            }
        }
...
pub enum ProviderKind {
    ...
    /// mode" (try POSEIDON without a real tracker) and the end-to-end tests. A
...
pub struct AzureDevOpsAuth {
    ...
    /// `POSEIDON_AZURE_PAT`; override per-project when polling multiple orgs
...
impl Default for AzureDevOpsAuth {
    fn default() -> Self {
        Self {
            pat_env: "POSEIDON_AZURE_PAT".to_string(),
        }
    }
}
...
pub struct TeamConfig {
    /// The team's display name - used throughout POSEIDON (the scope selector,
    ...
    /// are pulled - this is what lets two POSEIDON teams share one Azure DevOps
    ...
    /// Optional WIQL override. When absent, POSEIDON builds a default query that
    ...
    /// [`PoseidonConfig::rules_for`]. It's a whole-ruleset override, not a field
...
pub struct RuleSet {
    ...
    /// for a second area / a rewrite - see `poseidon_ai::default_max_suggestions`). Set
    ...
    /// When true, flag an OPEN item whose parent is in a resolved/closed state - a
    /// closed parent implies its children are done, so a live child means the parent was
    /// closed prematurely or the child is orphaned. Needs the parent in the polled set;
    ...
    /// Service-catalog integration. When a catalog is synced (see
    /// `docs/design/catalog-integration.md`), `product:*` resolves from it - the
    /// `repo_tags` above become the manual OVERRIDE layer. Owner-wide (a portal is
    /// per-tenant, not per-team), so it's read from the owner's default ruleset.
    #[serde(default)]
...
/// How POSEIDON ingests a service catalog. The `CatalogSource` impl (CSV/Port/…)
/// lives in `poseidon-providers`; this is the per-owner config that selects + maps
/// it. Secrets (a Port token) stay in the environment, named here, never stored.
...
pub struct CatalogConfig {
    ...
    /// Env var naming the source URL (e.g. the Port API base). Config, not secret.
    #[serde(default)]
    pub url_env: Option<String>,
    /// Env var naming the source token/credential. Read from env, never persisted.
    #[serde(default)]
    pub token_env: Option<String>,
    /// Which catalog columns carry which logical field (CSV / generic sources).
    #[serde(default)]
    pub field_map: Option<CatalogFieldMap>,
    /// Derive the allowed `product:*` set from the catalog instead of hand-listing.
    #[serde(default)]
    pub derive_allowed_products: bool,
}

/// Column/field mapping for a catalog source, mirrored into `poseidon-providers`'
/// `FieldMap` when the source is built (core can't depend on the provider crate).
#[derive(Debug, Clone, Default, PartialEq, Eq, serde::Serialize, serde::Deserialize)]
pub struct CatalogFieldMap {
    pub product: String,
    pub team: String,
...
///
/// ```
/// # use poseidon_core::parse_duration;
/// use std::time::Duration;
...
mod tests {
    ...
    fn user_config_parses_teams() {
        ...
        assert_eq!(cfg.teams[0].auth.pat_env, "POSEIDON_AZURE_PAT");
