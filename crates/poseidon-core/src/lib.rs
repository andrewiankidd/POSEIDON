//! POSEIDON's domain layer - provider-agnostic, IO-free types.
//!
...
//! (`PoseidonConfig` + `RuleSet`). What does NOT live here: any IO, any
...
///
/// Resolved in order: the `POSEIDON_VERSION` runtime env var (handy for testing
...
pub fn version() -> String {
    std::env::var("POSEIDON_VERSION")
        .ok()
        .filter(|v| !v.trim().is_empty())
        .or_else(|| option_env!("POSEIDON_VERSION").map(str::to_string))
...
/// from the `POSEIDON_COMMIT` runtime env var, then the build-time var (CI
...
pub fn commit() -> String {
    std::env::var("POSEIDON_COMMIT")
        .ok()
        .filter(|v| !v.trim().is_empty())
        .or_else(|| option_env!("POSEIDON_COMMIT").map(str::to_string))
