...
pub enum FlagCode {
    ...
    /// `poseidon_rules::is_underspecified`) - the most upstream hygiene problem, since
    ...
    NearDuplicate,
    /// Item is still open (New/Active/Blocked/…) but its PARENT is resolved/closed - a
    /// contradiction: a closed parent implies its children are done too, so either the
    /// parent was closed prematurely or this child is orphaned. Opt-in
    /// (`flag_orphaned_children`); needs the parent to be in the polled set.
    OrphanedChild,
