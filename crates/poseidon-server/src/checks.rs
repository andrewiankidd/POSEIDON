//! Concrete Doctor checks for POSEIDON.
//!
//! The [`poseidon_doctor`] crate is the generic engine; the checks that know
//! about POSEIDON's dependencies live here, where the config + provider are in
...
use poseidon_core::TeamConfig;
use poseidon_doctor::{Check, CheckResult, FixResult, Severity};

use crate::config_store::ConfigStore;

/// GitHub repo checked for new releases (`owner/name`).
const RELEASES_REPO: &str = "andrewiankidd/POSEIDON";
...
impl Check for UpdateCheck {
    ...
    async fn run(&self) -> CheckResult {
        let version = poseidon_core::version();
        if version == "local" {
        ...
        let result = Self::compute(&version, &poseidon_core::commit()).await;
...
/// Verifies POSEIDON can obtain a provider credential for one team -
