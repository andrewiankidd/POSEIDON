...
//!   POSEIDON_AUTH_JWKS_URL     the IdP's JWKS endpoint (enables verification)
//!   POSEIDON_AUTH_ISSUER       expected `iss` (optional but recommended)
//!   POSEIDON_AUTH_AUDIENCE     expected `aud` (optional but recommended)
//!   POSEIDON_AUTH_EMAIL_CLAIM  claim to read as the owner (default: `email`)
...
pub const ENV_JWKS_URL: &str = "POSEIDON_AUTH_JWKS_URL";
pub const ENV_ISSUER: &str = "POSEIDON_AUTH_ISSUER";
pub const ENV_AUDIENCE: &str = "POSEIDON_AUTH_AUDIENCE";
pub const ENV_EMAIL_CLAIM: &str = "POSEIDON_AUTH_EMAIL_CLAIM";
...
impl TokenVerifier {
    /// Build from the environment. `None` when `POSEIDON_AUTH_JWKS_URL` is unset,
