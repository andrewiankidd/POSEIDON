...
impl Scheduler {
    ...
    pub fn spawn(service: SharedService) -> Self {
        // `POSEIDON_NO_POLL` freezes the instance on whatever is already in the
        ...
        let frozen = std::env::var_os("POSEIDON_NO_POLL").is_some();

        let poll_svc = service.clone();
        let handle = tokio::spawn(async move {
            if frozen {
                info!("POSEIDON_NO_POLL set - scheduler frozen, not polling");
                return;
            }
