//! POSEIDON web instance binary - the Dockerised server.
//!
...
//! [`poseidon_server::serve`], which owns the store, scheduler, and HTTP loop.
...
async fn main() -> anyhow::Result<()> {
    ...
    let _telemetry = poseidon_server::init_telemetry(
        &config.telemetry,
        "poseidon-server",
        paths.log_dir(),
        false,
    ...
    let static_dir = poseidon_server::static_dir();

    tracing::info!(db = %db_path.display(), "using database");
    poseidon_server::serve(config, &db_path, &static_dir, &paths.az_sessions_dir()).await
}
