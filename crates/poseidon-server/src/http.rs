...
pub fn router(service: SharedService, static_dir: &Path) -> Router {
    let api = Router::new()
        ...
        .route("/api/work-items/{id}/mark-duplicate", post(mark_duplicate))
...
async fn health() -> impl IntoResponse {
    ...
    Json(serde_json::json!({ "status": "ok", "service": "poseidon", "version": build_version() }))
}
...
/// before app.js) and reads `window.__POSEIDON_ENV__`. Values are resolved from
/// env at *request* time, so one static bundle serves any deployment - no build.
/// Currently exposes `instanceUrl` (from `POSEIDON_REMOTE_URL`): when set, the
...
/// The build/version stamp for this running binary. CI passes POSEIDON_COMMIT (git
/// sha) + POSEIDON_VERSION (tag); a local `poseidon.sh` build passes a timestamp as
/// POSEIDON_COMMIT. Prefer the commit/build stamp, else the version tag, else "dev";
...
fn build_version() -> String {
    let commit = std::env::var("POSEIDON_COMMIT").unwrap_or_default();
    let ver = std::env::var("POSEIDON_VERSION").unwrap_or_default();
...
async fn env_js() -> impl IntoResponse {
    ...
    let env = serde_json::json!({
        "instanceUrl": std::env::var("POSEIDON_REMOTE_URL").unwrap_or_default(),
        "version": version,
    });
    (
        [
        ...
        format!("window.__POSEIDON_ENV__ = {env};\n"),
    )
}

/// Whether POSEIDON can authenticate (PAT or `az`), for the sign-in banner.
...
async fn add_team(svc: Scoped, Json(team): Json<poseidon_core::TeamConfig>) -> ApiResult {
...
async fn update_work_item(
    ...
    let update = poseidon_core::WorkItemUpdate {
        state: body.state,
        tags: body.tags,
    };
...
/// Body for marking a work item as a duplicate of another.
#[derive(serde::Deserialize)]
struct MarkDuplicateBody {
    team: String,
    duplicate_of: i64,
}
...
async fn mark_duplicate(
    svc: Scoped,
    axum::extract::Path(id): axum::extract::Path<i64>,
    Json(body): Json<MarkDuplicateBody>,
) -> ApiResult {
    match svc
        .mark_work_item_duplicate(&body.team, id, body.duplicate_of)
        .await
    {
        Ok((item, flags)) => Ok(Json(serde_json::json!({ "item": item, "flags": flags }))),
        Err(e) => Err(err500(e)),
...
async fn update_rules(svc: Scoped, Json(rules): Json<poseidon_core::RuleSet>) -> ApiResult {
...
async fn update_team_rules(
    ...
    Json(rules): Json<poseidon_core::RuleSet>,
) -> ApiResult {
...
async fn update_team(
    svc: Scoped,
    axum::extract::Path(name): axum::extract::Path<String>,
    Json(team): Json<poseidon_core::TeamConfig>,
) -> ApiResult {
...
async fn run_report_spec(
    svc: Scoped,
    Query(q): Query<ScopeQuery>,
    Json(spec): Json<poseidon_core::ReportSpec>,
) -> ApiResult {
...
async fn save_report(svc: Scoped, Json(spec): Json<poseidon_core::ReportSpec>) -> ApiResult {
        ...
        Ok(yaml) => (
            [
                ...
                (
                    ...
                    "attachment; filename=\"poseidon-config.yaml\"",
                ),
            ],
            yaml,
        )
...
/// The owner's synced service catalog (repo -> product/team rows + a count), for the
/// Settings "Catalog" panel to show current coverage.
async fn catalog_get(svc: Scoped) -> ApiResult {
    let rows = svc.catalog().await.map_err(err500)?;
    Ok(Json(
        serde_json::json!({ "count": rows.len(), "rows": rows }),
    ))
}

/// Import a service catalog from an uploaded CSV export (the CSV `CatalogSource`).
/// Replaces the owner's catalog snapshot; returns how many repo-keyed rows were stored.
async fn catalog_import(svc: Scoped, body: String) -> ApiResult {
    let rows = svc.sync_catalog_csv(&body).await.map_err(err500)?;
...
async fn llm_config_set(svc: Scoped, Json(cfg): Json<poseidon_ai::LlmConfig>) -> ApiResult {
...
async fn llm_config_autotune(
    svc: Scoped,
    Json(caps): Json<poseidon_ai::PlatformCaps>,
) -> ApiResult {
