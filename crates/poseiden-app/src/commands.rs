...
pub async fn update_work_item(
    ...
    let update = poseiden_core::WorkItemUpdate { state, tags };
...
pub async fn store_tag_suggestions(
    state: State<'_, AppState>,
    team: Option<String>,
    items: Vec<poseiden_server::BrowserSuggestion>,
) -> CmdResult {
    let service = state.service()?;
    let summary = service
...
pub async fn get_reports(
    ...
    let service = state.service()?;
    let (from, to) = poseiden_server::normalise_range(from.as_deref(), to.as_deref());
    let team = scope(&team);
    let tickets = service
