//! The shared `Service` - POSEIDEN's business logic, transport-agnostic.
//!
...
/// Whether POSEIDEN can currently authenticate to the provider, and how. Drives
...
impl Service {
    ...
    async fn stored_llm_config(&self) -> poseiden_ai::LlmConfig {
        if let Ok(Some(json)) = self.store.get_meta(&self.owner, "llm_config").await {
            if let Ok(cfg) = serde_json::from_str::<poseiden_ai::LlmConfig>(&json) {
                return cfg;
            }
        }
    ...
    async fn ai_tagger(&self) -> Option<Arc<dyn poseiden_ai::AiTagger>> {
        {
    ...
    pub async fn llm_config_view(&self) -> serde_json::Value {
        ...
        let caps = poseiden_ai::PlatformCaps::server();
        ...
        serde_json::json!({
            ...
            "caps": caps,
            "active_id": active,
            "presets": { "online": poseiden_ai::ONLINE_PROVIDERS, "offline": poseiden_ai::OFFLINE_MODELS },
        })
    }
    ...
    pub async fn benchmark_llms(&self) -> serde_json::Value {
        ...
        let cfg = self.stored_llm_config().await;
        let caps = poseiden_ai::PlatformCaps::server();
        let item = poseiden_ai::TaggerInput {
            id: 0,
    ...
    pub async fn import_config(&self, yaml: &str, replace: bool) -> anyhow::Result<ImportSummary> {
        ...
        if bundle.poseiden.schema > CONFIG_BUNDLE_SCHEMA {
            anyhow::bail!(
                "this config needs a newer POSEIDEN (bundle schema {}, this build supports {})",
                bundle.poseiden.schema,
    ...
    pub async fn update_rules(&self, rules: poseiden_core::RuleSet) -> anyhow::Result<()> {
    ...
    pub async fn update_team_rules(
        &self,
        team: &str,
        rules: Option<poseiden_core::RuleSet>,
    ) -> anyhow::Result<bool> {
    ...
    /// Whether POSEIDEN can authenticate right now, and how. A PAT on any team
    ...
    pub async fn start_web_sign_in(&self) -> anyhow::Result<poseiden_providers::DeviceCode> {
        use poseiden_providers::oauth::{self, PollOutcome};
        let tenant = self.primary_tenant().await;
        ...
        let prompt = poseiden_providers::DeviceCode {
    ...
    async fn build_team_provider(
        &self,
        team_cfg: &TeamConfig,
    ) -> Result<Box<dyn poseiden_providers::Provider>, String> {
        ...
        let credential = match team_cfg.provider {
            ...
            ProviderKind::Stub => poseiden_providers::Credential::Pat(String::new()),
    ...
    pub async fn pull_requests(&self, team: Option<&str>) -> anyhow::Result<Vec<PullRequest>> {
        ...
        for pr in &mut prs {
            ...
            pr.flags = poseiden_rules::evaluate_pull_request(pr, &rules.pull_requests, now);
        }
        Ok(prs)
    }
    ...
    pub async fn report_specs(&self) -> anyhow::Result<Vec<poseiden_core::ReportSpec>> {
        let mut specs = poseiden_reports::builtins();
    ...
    ) -> anyhow::Result<poseiden_core::ReportResult> {
    ...
    pub async fn run_report_spec(
        &self,
        mut spec: poseiden_core::ReportSpec,
        team_override: Option<&str>,
    ) -> anyhow::Result<poseiden_core::ReportResult> {
        ...
        Ok(poseiden_reports::run(&spec, &data, Utc::now()))
    }
    ...
    ) -> anyhow::Result<poseiden_reports::Datasets> {
        use poseiden_core::DataSource;
        let team = spec.team.as_deref();
        let uses = |src: DataSource| spec.series.iter().any(|s| s.source == src);
        let mut data = poseiden_reports::Datasets::default();
    ...
    pub async fn delete_report(&self, name: &str) -> anyhow::Result<bool> {
        if poseiden_reports::builtins().iter().any(|b| b.name == name) {
...
fn report_runs_since(range: &poseiden_core::TimeRange) -> chrono::DateTime<Utc> {
    let days = match range {
        poseiden_core::TimeRange::LastDays { days } => days + 2,
        _ => 3650,
    };
...
impl From<poseiden_providers::oauth::TokenSet> for CachedToken {
    fn from(t: poseiden_providers::oauth::TokenSet) -> Self {
        Self {
...
pub(crate) async fn acquire_cached_token(
    ...
    match poseiden_providers::oauth::refresh(&reqwest::Client::new(), tenant, rt).await {
        Ok(ts) => {
...
fn fold_pipeline_health(
    pipelines: &[poseiden_core::Pipeline],
    runs: &[poseiden_core::PipelineRun],
) -> Vec<PipelineHealth> {
    // Runs arrive newest-first (the store orders by started_at DESC); rely on
    // that for "last run" / "last status".
    let mut by_pipeline: HashMap<i64, Vec<&poseiden_core::PipelineRun>> = HashMap::new();
    for r in runs {
...
mod tests {
    use super::*;
    use poseiden_core::{Pipeline, PipelineRun};
    ...
    fn config_bundle_yaml_round_trips() {
        use poseiden_core::{BundleMeta, ConfigBundle, UserConfig, CONFIG_BUNDLE_SCHEMA};
        let bundle = ConfigBundle {
            poseiden: BundleMeta {
                schema: CONFIG_BUNDLE_SCHEMA,
        ...
        // Envelope under `poseiden:`; the owner config is flattened to the top
        ...
        assert_eq!(back.poseiden.schema, CONFIG_BUNDLE_SCHEMA);
        assert_eq!(back.poseiden.app_version, "test");
        assert_eq!(back.poseiden.owner.as_deref(), Some("someone@example.com"));
        assert!(back.config.poll_all_teams);

        // Owner is optional: a bundle without it deserializes to None.
        let minimal: ConfigBundle = serde_norway::from_str("poseiden:\n  schema: 1\n").unwrap();
        assert_eq!(minimal.poseiden.owner, None);
    ...
    async fn bundle_owner_ignored_by_public_import_but_honored_by_trusted() {
        use poseiden_core::DEFAULT_OWNER;
        let store = Store::connect_in_memory().await.unwrap();
        let svc = Service::new(PoseidenConfig::default(), store, std::env::temp_dir());
    ...
    use poseiden_core::{RuleSet, TagKeywords, UserConfig};

    /// A `Service` over a fresh in-memory store, owner = `default`.
    async fn test_service() -> Service {
        let store = Store::connect_in_memory().await.unwrap();
        Service::new(PoseidenConfig::default(), store, std::env::temp_dir())
    }
    ...
    async fn evaluate_scoped_judges_each_team_by_its_effective_rules() {
        ...
        let cfg = UserConfig {
            teams: vec![
                TeamConfig {
                    name: "Alpha".into(),
                    provider: poseiden_core::ProviderKind::Stub,
    ...
    impl poseiden_ai::AiTagger for StubTagger {
        async fn suggest(
            &self,
            ...
            Ok(allowed
                .first()
                .map(|t| poseiden_core::TagSuggestion {
                    tag: t.clone(),
