...
use poseiden_core::PoseidenConfig;
...
/// `cargo run`.
pub const ENV_STATIC_DIR: &str = "POSEIDEN_STATIC_DIR";
...
/// lives in the DB - set via the UI, or `poseiden config import` for headless /
...
pub fn load_config() -> PoseidenConfig {
    PoseidenConfig {
...
/// exporters spawn background tasks); every POSEIDEN binary is.
pub fn init_telemetry(
    settings: &poseiden_telemetry::TelemetrySettings,
    service_name: &str,
    log_dir: PathBuf,
    console_stderr: bool,
) -> Option<poseiden_telemetry::TelemetryGuard> {
    let environment = if poseiden_core::is_local_build() {
        poseiden_telemetry::Environment::Dev
    } else {
        poseiden_telemetry::Environment::Prod
    };
    let rt = poseiden_telemetry::RuntimeInfo::new(
        service_name,
        poseiden_core::version(),
        environment,
        log_dir,
    )
    .with_console_stderr(console_stderr);
    match poseiden_telemetry::init(settings, rt) {
        Ok(guard) => Some(guard),
        Err(e) => {
...
pub async fn serve(
    ...
    // Optional defence-in-depth: when POSEIDEN_AUTH_JWKS_URL is set, verify the
    ...
    info!(%addr, static_dir = %static_dir.display(), "POSEIDEN server listening");
