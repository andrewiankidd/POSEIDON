...
/// One GitLab project POSEIDON polls. The project path (`namespace/project`)
...
impl GitlabProvider {
    pub fn new(cfg: &TeamConfig, credential: Credential) -> Result<Self, ProviderError> {
        ...
        let mut builder = reqwest::Client::builder().user_agent("poseidon");
...
impl Provider for GitlabProvider {
    ...
    async fn mark_duplicate(&self, id: i64, duplicate_of: i64) -> Result<WorkItem, ProviderError> {
        // GitLab's `/duplicate #N` quick action (posted as a note body) records the
        // issue as a duplicate of the target AND closes it, in one step - the native
        // "mark duplicate". Requires the token to have write (Reporter+) on the project.
        let url = self.project_url(&format!("issues/{id}/notes"), "");
        let resp = self
            .client
            .post(&url)
            .json(&serde_json::json!({ "body": format!("/duplicate #{duplicate_of}") }))
            .send()
            .await?;
        let status = resp.status();
        if !status.is_success() {
        ...
        // The note response isn't the issue; re-fetch to return the post-write view.
        let issue: GlIssue = self
            .get_json(&self.project_url(&format!("issues/{id}"), ""))
            .await?;
        Ok(normalise_issue(issue, &self.team))
    }
}
...
/// Map a GitLab issue state to POSEIDON's state string. "opened" -> "Open",
...
/// Map a GitLab merge-request state to POSEIDON's [`PrStatus`]: "opened" (or the
...
/// Map a GitLab pipeline status into POSEIDON's [`RunStatus`]. GitLab uses a
...
mod tests {
    ...
    async fn live_anonymous_fetch_is_non_empty() {
        let cfg = TeamConfig {
            name: "Runner".into(),
            provider: poseidon_core::ProviderKind::GitLab,
