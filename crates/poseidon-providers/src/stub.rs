...
//!
//! Two jobs: **demo mode** (try POSEIDON with a realistic backlog and zero
...
impl Provider for StubProvider {
                ...
                1002 => {
                    ...
                    it.description = Some(
                        "Document the YAML config-import format consumed by \
                         `poseidon config import`, including the rules schema."
                            .into(),
                    );
                }
                _ => {}
    ...
    async fn mark_duplicate(&self, id: i64, _duplicate_of: i64) -> Result<WorkItem, ProviderError> {
        // Demo mode: echo the item so the action succeeds without a real backend.
    ...
    async fn editable_fields(
        &self,
        id: i64,
    ) -> Result<Vec<poseidon_core::EditableField>, ProviderError> {
        use poseidon_core::{EditableField, FieldKind};
        let item = self
            .fetch_work_items()
            .await?
            .into_iter()
    ...
    async fn update_fields(
        &self,
        id: i64,
        changes: &[poseidon_core::FieldChange],
    ) -> Result<WorkItem, ProviderError> {
