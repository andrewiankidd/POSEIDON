...
/// and fields POSEIDON already edits with a purpose-built control (State, Tags are
...
impl Provider for AzureDevOpsProvider {
    ...
    async fn mark_duplicate(&self, id: i64, duplicate_of: i64) -> Result<WorkItem, ProviderError> {
        // Add a "Duplicate Of" link from `id` to `duplicate_of` - the reverse side of
        // the Duplicate link type, so `id` reads "Duplicate Of" the canonical item and
        // ADO auto-creates the inverse "Duplicate" on the other. The relation url is the
        // target's own REST url (its `url` field), which ADO returns on a plain GET. We
        // deliberately DON'T auto-close: the closed state/reason varies by process, and
        // linking is the canonical, always-valid mark; closing stays a separate, explicit
        // State edit the user makes if they want.
        let target: serde_json::Value = self
            .get_json(&self.project_url(&format!("wit/workitems/{duplicate_of}"), ""))
            .await?;
        let url = target.get("url").and_then(|u| u.as_str()).ok_or_else(|| {
            ProviderError::Config(format!("could not resolve work item {duplicate_of}"))
        })?;
        let ops = vec![serde_json::json!({
            "op": "add",
            "path": "/relations/-",
            "value": {
                ...
                "url": url,
                "attributes": { "comment": "Marked duplicate via POSEIDON" }
            }
        })];
        self.patch_work_item(id, ops).await
    }
...
// ─────────────────────────── Azure DevOps DTOs ───────────────────────────
// Only the fields POSEIDON consumes. `Option` everywhere the API might omit a
...
fn normalise_work_item(
    ...
    WorkItem {
        ...
        // empty Description and the real body in Repro Steps; but when BOTH are populated
        // (e.g. a symptom in Description and the raised-in-support origin in Repro Steps),
        // keeping only one drops signal the tagger / underspecified check need. Join the
        // non-empty parts so nothing is lost.
        description: {
            let d = f
                .description
                .as_deref()
...
fn strip_html(html: &str) -> String {
    ...
    for ch in html.chars() {
        match ch {
            '<' => in_tag = true,
            // A tag boundary is a WORD boundary: emit a space so removing `</a><br>`
            // between "channel" and "June" yields "channel June", not "channelJune" -
            // otherwise multi-word keyword matching (e.g. "support channel") breaks and
            // adjacent words fuse. Runs of spaces are collapsed by split_whitespace below.
            '>' => {
                in_tag = false;
                out.push(' ');
            }
...
/// Map Azure DevOps' split status/result into POSEIDON's single [`RunStatus`].
...
mod tests {
    ...
    #[test]
    fn strip_html_separates_words_at_tag_boundaries() {
        // Tags removed between words must leave a space; otherwise "channel</a><br>June"
        // fuses to "channelJune" and multi-word keyword matching ("support channel") fails.
        let s = strip_html("<div>from support channel</div><a href=\"x\">June</a><br>site-updater");
        assert!(s.contains("support channel June"), "{s}");
        assert!(!s.contains("channelJune"), "{s}");
        assert!(!s.contains("Junesite"), "{s}");
        // Plain text is unchanged (aside from whitespace collapse).
    ...
    fn normalise_work_item_falls_back_to_repro_steps_for_bugs() {
        ...
        // When BOTH are present the body COMBINES them (nothing dropped) - a symptom in
        // Description and e.g. the raised-in-support origin in Repro Steps both survive,
        // so a keyword in either still reaches the tagger.
