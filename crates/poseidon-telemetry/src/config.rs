...
//! be written).
//!
//! The split keeps the crate app-agnostic: nothing here knows it's POSEIDON.
...
impl TelemetrySettings {
    ...
    ///
    /// - `POSEIDON_LOG_CONSOLE` / `POSEIDON_LOG_CONSOLE_JSON` / `POSEIDON_LOG_FILE` - bools
    /// - `POSEIDON_LOG_LEVEL` - explicit `RUST_LOG`-syntax filter
    /// - `POSEIDON_OTLP_ENDPOINT` - set it to enable OTLP export to that collector
    /// - `POSEIDON_OTLP_ENABLED` - force-enable/disable independently of the endpoint
    pub fn from_env() -> Self {
        ...
        s.console = flag("POSEIDON_LOG_CONSOLE", s.console);
        s.console_json = flag("POSEIDON_LOG_CONSOLE_JSON", s.console_json);
        s.file = flag("POSEIDON_LOG_FILE", s.file);
        s.level = std::env::var("POSEIDON_LOG_LEVEL")
            .ok()
            .filter(|v| !v.trim().is_empty());
        if let Ok(ep) = std::env::var("POSEIDON_OTLP_ENDPOINT") {
            if !ep.trim().is_empty() {
                s.otlp.endpoint = ep;
                s.otlp.enabled = true;
            }
        }
        s.otlp.enabled = flag("POSEIDON_OTLP_ENABLED", s.otlp.enabled);
        s
    }
}
...
/// the local `grafana/otel-lgtm` stack) via `POSEIDON_OTLP_ENDPOINT`.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(default)]
...
pub struct RuntimeInfo {
    /// `service.name` on every span/log/metric (e.g. `"poseidon-server"`).
    ...
    /// the host resolves it (POSEIDON uses `Paths::log_dir()`).
    pub log_dir: PathBuf,
