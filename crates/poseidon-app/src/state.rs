...
//!
//! POSEIDON's first run asks the user *how* to run (local vs a remote instance)
...
use poseidon_paths::{enable_portable_sentinel, Paths};
use poseidon_server::{Scheduler, Service, SharedService};
...
struct Runtime {
    ...
    _telemetry: Option<poseidon_telemetry::TelemetryGuard>,
}
...
impl AppState {
    ...
    pub fn boot() -> Self {
        ...
        if paths.database_path().exists() {
            match tauri::async_runtime::block_on(build_runtime(paths.is_portable())) {
                ...
                Err(e) => eprintln!("POSEIDON: failed to open existing store: {e}"),
            }
        }
        Self { inner }
    }
    ...
    pub fn service(&self) -> Result<SharedService, String> {
        self.inner
            ...
            .ok_or_else(|| "POSEIDON is not set up yet".to_string())
    }
...
async fn build_runtime(portable: bool) -> anyhow::Result<Runtime> {
    ...
    let telemetry =
        poseidon_server::init_telemetry(&config.telemetry, "poseidon-app", paths.log_dir(), false);
    if portable {
