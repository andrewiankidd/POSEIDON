//! Portable-first path resolution for POSEIDEN.
//!
...
//!
//! 1. **Portable** - if `POSEIDEN_PORTABLE_MODE=true` OR a `.portable` sentinel
...
//!    data dir (`~/.local/share/poseiden/` on Linux, `%APPDATA%\poseiden\` on
//!    Windows, `~/Library/Application Support/poseiden/` on macOS). In the
...
pub const ENV_PORTABLE: &str = "POSEIDEN_PORTABLE_MODE";
...
pub const ENV_DATA_DIR: &str = "POSEIDEN_DATA_DIR";
...
/// Resolved, confined set of writable locations for one POSEIDEN run.
#[derive(Debug, Clone)]
pub struct Paths {
    data_root: PathBuf,
    portable: bool,
...
impl Paths {
    ...
    /// SQLite database file: `<root>/poseiden.db`.
    pub fn database_path(&self) -> PathBuf {
        self.data_root.join("poseiden.db")
    }
...
pub fn enable_portable_sentinel() -> std::io::Result<PathBuf> {
    ...
    std::fs::write(
        &sentinel,
        b"POSEIDEN portable mode.\nDelete this file to return to OS-standard storage.\n",
    )?;
    Ok(sentinel)
}

/// OS-standard data directory for POSEIDEN. Falls back to `./poseiden-data`
...
fn os_data_dir() -> PathBuf {
    directories::ProjectDirs::from("", "", "poseiden")
        .map(|d| d.data_dir().to_path_buf())
        .unwrap_or_else(|| PathBuf::from("poseiden-data"))
}
...
mod tests {
    ...
    fn portable_env_flag_roots_under_binary_dir() {
        let bin = PathBuf::from("/opt/poseiden");
        ...
        assert_eq!(paths.database_path(), bin.join(".portable/poseiden.db"));
    ...
    fn data_dir_override_wins_over_os_dir_but_is_not_portable() {
        ...
        assert_eq!(paths.database_path(), Path::new("/data/poseiden.db"));
    }
    ...
    fn portable_beats_data_dir_override() {
        ...
        let paths = Paths::resolve_with(&env, Some(Path::new("/opt/poseiden")));
        assert!(paths.is_portable());
        assert_eq!(paths.data_root(), Path::new("/opt/poseiden/.portable"));
    }
