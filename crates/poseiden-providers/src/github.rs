...
/// One GitHub repository POSEIDEN polls. `owner`/`repo` come from the team's
...
impl Provider for GithubProvider {
    ...
    async fn fetch_pipelines(&self) -> Result<Vec<Pipeline>, ProviderError> {
        // Actions workflows are POSEIDEN's pipelines. The workflows endpoint does
    ...
    async fn update_work_item(
        ...
        // GitHub write-back is out of scope: POSEIDEN reads GitHub, it does not
        // edit issues here.
...
// ─────────────────────────── GitHub DTOs ───────────────────────────
// Only the fields POSEIDEN consumes. `Option`/`default` everywhere the API might
...
/// Map a GitHub issue `state` ("open"/"closed") to POSEIDEN's title-cased form.
...
/// Map a GitHub PR's merge/close state into POSEIDEN's [`PrStatus`]. Merged wins
...
/// Map an Actions run's `status` + `conclusion` into POSEIDEN's [`RunStatus`].
...
mod tests {
    ...
    fn sample_cfg() -> TeamConfig {
        TeamConfig {
            name: "OSS".into(),
            provider: poseiden_core::ProviderKind::GitHub,
    ...
    // `cargo test -p poseiden-providers -- --ignored github_live`.
