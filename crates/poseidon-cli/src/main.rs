//! POSEIDON CLI - `poseidon`.
//!
//! Drives the exact same [`poseidon_server::Service`] the GUI and web instance
...
//! env var is set - including CI, where `poseidon lint` turns backlog hygiene
//! into a check.
...
use poseidon_core::Severity;
use poseidon_paths::Paths;
use poseidon_server::{Service, SharedService};
...
#[command(
    name = "poseidon",
    version,
    about = "POSEIDON - a Product Owner support tool for backlog hygiene + flow."
)]
struct Cli {
...
async fn main() -> anyhow::Result<()> {
    ...
    // bare `poseidon`, `--help`, or a usage error (clap exits during parse for
...
async fn build_service(
) -> anyhow::Result<(SharedService, Option<poseidon_telemetry::TelemetryGuard>)> {
    ...
    let config = poseidon_server::load_config();
    // Bring telemetry up before any work; stderr console keeps stdout clean.
    let telemetry =
        poseidon_server::init_telemetry(&config.telemetry, "poseidon-cli", paths.log_dir(), true);
